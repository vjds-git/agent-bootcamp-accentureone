...
# Specify the desired order of tool calling for the agent
...
def _safe_dt(x) -> Optional[datetime]:
    ...
    if isinstance(x, datetime):
        return x
    # Some SDKs may return ISO strings:
    try:
...
def list_traces_for_session(langfuse, session_id: str, limit: int = 100) -> List[str]:
    ...
    Uses cursor-based pagination if available, otherwise falls back to page-based.
    """
    trace_ids: List[str] = []

    # Cursor-based pagination (preferred in newer SDKs)
    cursor = None
    page = 1
    ...
    while True:
        try:
            resp = langfuse.api.trace.list(
                session_id=session_id,   # traces endpoint supports sessionId filtering [3](https://github.com/orgs/langfuse/discussions/9479)
                limit=limit,
                cursor=cursor,           # may be supported depending on SDK version [4](https://langfuse.com/docs/api-and-data-platform/features/query-via-sdk)
            )
        except TypeError:
            # Older SDK: page-based pagination
            resp = langfuse.api.trace.list(
        ...
        # If page-based: stop when fewer than limit returned
        if len(data) < limit:
            break
        page += 1
...
def extract_agent_tool_calls_for_session(
    ...
    *,
    ...
    """
    langfuse = get_client()  # uses env vars for auth [4](https://langfuse.com/docs/api-and-data-platform/features/query-via-sdk)
    ...
    for trace_id in trace_ids:
        # Fetch the full trace including observations [5](https://langfuse.com/guides/cookbook/example_query_data_via_sdk)
        trace = langfuse.api.trace.get(trace_id)

        observations = getattr(trace, "observations", None) or []
        for obs in observations:
            # obs_type = _normalize_type(getattr(obs, "name", None))

            # # tool calls are observation type "tool" [2](https://langfuse.com/docs/observability/features/observation-types)
            # if obs_type != "tool":
            #     continue
    ...
    # Sort across all traces in the session by time (oldest → newest)
...
def plot_tool_traces(trace_df):
    ...
    # Create colors 
    cmap = plt.cm.Blues_r  # dark -> light (because reversed)
    n = len(trace_df)
...
if __name__ == "__main__":
    # _main() # Creates the traces csv file
